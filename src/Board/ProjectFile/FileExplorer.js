...
function FileItem({ name, created, size, selectedProjectName, succ, setSucc }) {
    ...
    const handleDownloadFile = (filename) => {
        const url = `http://14.58.108.70:8877/download/${encodeURIComponent(filename)}?Project=${encodeURIComponent(selectedProjectName)}`;
    ...
    const handleDeleteFile = (filename) => {
        Axios.delete(`http://14.58.108.70:8877/deleteFile/${encodeURIComponent(filename)}?Project=${encodeURIComponent(selectedProjectName)}`)
        .then((response) => {
...
function FileExplorer({ selectedProjectName }) {
    ...
    const LoadFiles = async () => {
        try {
            const response = await Axios.get(`http://14.58.108.70:8877/getFile?Project=${encodeURIComponent(selectedProjectName)}`);
    ...
    const uploadFile = async (file) => {
        ...
        try {
            ...
            const response = await Axios.post(`http://14.58.108.70:8877/uploadFile?Project=${encodeURIComponent(projectName)}&dateTime=${encodeURIComponent(dateTime)}`, formData, {
                headers: {
