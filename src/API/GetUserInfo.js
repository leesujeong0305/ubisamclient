...
function GetUserInfo(userEmail ,authUserTeam){
    ...
    if (authUserTeam === undefined) {
        return Axios.get(`${ip}/getUserInfo?id=${encodeURIComponent(emailToken)}&name=${encodeURIComponent(token)}`, { //get은 body없음
            headers: {
        ...
        return Axios.get(`${ip}/getUserInfo?id=${encodeURIComponent(userEmail)}&name=${encodeURIComponent(authUserTeam)}`, { //get은 body없음
            headers: {
                "Content-Type": "application/json",
            }
        }).then((res) => {
            if (userEmail === 'All' && authUserTeam === 'All') {
                return res.data;
            }
            if (res.data) {
